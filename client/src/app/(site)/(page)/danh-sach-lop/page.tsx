...
import { ClassCreate, ClassItem } from '@/types';

type ModalMode = 'create' | 'edit';
export default function DanhSachLopPage() {

  const [query, setQuery] = useState('');
  const { data, error, isLoading, mutate } = useClasses();

  const [isOpen, setIsOpen] = useState(false);
  const [mode, setMode] = useState<ModalMode>('create');
  const [editingId, setEditingId] = useState<number | null>(null);
  const [form, setForm] = useState<ClassCreate>({ className: '', fullNameTeacher: '', description: '' });
  const list = data ?? [];
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((d) =>
      [d.id + '', d.className, d.fullNameTeacher, d.description ?? ''].some((v) => v.toLowerCase().includes(q))
    );
  }, [list, query]);
  ...
  function openEdit(item: ClassItem) {
    ...
    setForm({ className: item.className, fullNameTeacher: item.fullNameTeacher, description: item.description ?? '' });
    setIsOpen(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'create') {
      await mutate(async (current) => {
        ...
        return [created, ...((current as ClassItem[]) ?? [])];
  ...
  async function handleDelete(id: number) {
    ...
    await mutate(async (current) => {
      ...
      return ((current as ClassItem[]) ?? []).filter((c) => c.id !== id);
    }, { revalidate: false, rollbackOnError: true });
  }
...
// Component Modal thuần Tailwind (không dùng thư viện ngoài)
